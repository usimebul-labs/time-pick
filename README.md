...
#### 결과 화면
1. 결과 화면 구성은 다음과 같은 순서로 이루어짐
1-2. 월간 캘린더의 경우 1. 참여자 목록, 2. 시간 설정, 3 1 ~ 3순위 일정 목록, 4. 기타 안내 정보, 5. 확정 버튼
1-3. 주간 캘린더의 경우 1. 참여자 목록, 2. 소요 시간 설정 (1시간 단위), 3. 1 ~ 3순위 일정 목록, 4. 기타 안내 정보, 5. 확정 버튼

2. 참여자 목록에 참여자들은 복수 선택이 가능하고, 해당 참여자들이 모두 참여할 수 있는 일정으로 1 ~ 3 순위 일정 재산정
3. 월간 캘린더 시간 설정은 단순히 해당 날짜의 시간을 설정하기 위한 것
4. 주간 캘린더의 소요 시간 설정은 해당 일정을 소화하는데 필요한 연속된 시간으로, 이 시간을 참고해서 1 ~ 3 순위 일정을 재산정함
5. 1 ~ 3 순위 일정에서 1순위만 기본적으로 보여지는 상태고 나머지 순위는 펼치기 기능을 통해 확인 가능
6. 1 ~ 3 순위 일정에서 각 일정은 선택하여 확정하고자 하는 일정을 정함
5. 기타 안내 정보는 기본 입력 항목 (장소, 교통, 주차, 회비, 입금, 문의, 메모)을 제공하고 임의로 항목을 추가할 수 있음
7. 기타 안내 정보는 모두 선택 사항이지만 장소 정보는 바로 입력할 수 있게 하고 지도 API를 연동
8. 확정 버튼을 눌러 해당 일정을 확정하면





결과 화면 구성은 일정 제목, 설명, 일정 기간 및 마감일, 참여자 목록, 참여자 수 차트, 일자별 상세란, 다시 수정하기 버튼, 완료 버튼