            ...
            <div className="space-y-4">
                {displayedSchedules.length > 0 ? (
                    displayedSchedules.map((schedule) => (
                        <div
                            key={schedule.id}
                            className={`bg-white rounded-2xl p-5 shadow-sm border border-gray-100 cursor-pointer transition-all hover:shadow-md hover:border-indigo-100 active:scale-[0.99] ${type === 'my' ? 'group relative' : ''}`}
                    ...
                    <div className="flex flex-col items-center justify-center py-10 bg-white rounded-2xl border border-dashed border-gray-200">
                        <div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center mb-3">
                            {type === 'my' ? <Calendar className="w-6 h-6 text-gray-300" /> : <Users className="w-6 h-6 text-gray-300" />}
                        </div>
